...
func TestCreateRideRequestBodyUsesApplicationOwnedContract(t *testing.T) {
	...
	input, ok := body.input()
	if !ok {
		t.Fatal("input reported complete request as incomplete")
	}
	if input.BookingMode != ride.BookingModeOffers {
		t.Fatalf("unexpected booking mode: %q", input.BookingMode)
	}
	if input.ProposedFare == nil || input.ProposedFare.AmountMinor != 70000 || input.ProposedFare.Currency != "PKR" {
		t.Fatalf("unexpected fare: %#v", input.ProposedFare)
	}
}
...
func TestCreateRideRequestBodyAllowsPresentZeroCoordinates(t *testing.T) {
	...
	input, ok := body.input()
	if !ok {
		t.Fatal("input rejected present zero coordinates")
	}
	if input.Pickup.Latitude != 0 || input.Pickup.Longitude != 0 || input.Destination.Latitude != 0 || input.Destination.Longitude != 0 {
