...
func (app application) registerDriverRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /v1/driver/trip", app.authenticated(app.getDriverCurrentTrip))
	mux.Handle("GET /v1/driver/marketplace/ride-requests", app.authenticated(app.discoverDriverMarketplace))
	...
	mux.Handle("POST /v1/driver/ride-requests/{ride_request_id}/complete", app.authenticated(app.completeTrip))
	mux.Handle("POST /v1/driver/ride-requests/{ride_request_id}/cancel", app.authenticated(app.cancelDriverRideRequest))
}
...
func (app application) registerRideRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /v1/ride-requests/{ride_request_id}", app.authenticated(app.getRideRequestStatus))
	mux.Handle("POST /v1/ride-requests/{ride_request_id}/cancel", app.authenticated(app.cancelRideRequest))
