...
func main() {
	...
	errCh:=make(chan error,1);go func(){logger.Info("API server starting","port",cfg.Port);errCh<-server.ListenAndServe()}()
	signalCtx,stop:=signal.NotifyContext(context.Background(),os.Interrupt,syscall.SIGTERM);defer stop()
	select{case<-signalCtx.Done():case err:=<-errCh:if !errors.Is(err,http.ErrServerClosed){logger.Error("server stopped unexpectedly","error",err);os.Exit(1)}}
...
func(app application)ready(w http.ResponseWriter,r *http.Request){ctx,cancel:=context.WithTimeout(r.Context(),2*time.Second);defer cancel();if err:=app.db.PingContext(ctx);err!=nil{writeJSON(w,http.StatusServiceUnavailable,map[string]string{"status":"unavailable"});return};writeJSON(w,http.StatusOK,map[string]string{"status":"ready"})}
func(app application)me(w http.ResponseWriter,r *http.Request){p,ok:=identity.PrincipalFromContext(r.Context());if !ok{writeJSON(w,http.StatusUnauthorized,map[string]string{"error":"authentication required"});return};u,err:=app.users.GetOrCreate(r.Context(),user.ExternalIdentity{Issuer:p.Issuer,Subject:p.Subject});if err!=nil{writeJSON(w,http.StatusInternalServerError,map[string]string{"error":"unable to load user"});return};writeJSON(w,http.StatusOK,map[string]any{"id":u.ID,"capabilities":u.Capabilities})}
