...
func TestRetirementPreservesLegacyRidesTripsAndOffers(t *testing.T) {
	...
	var before string
	const snapshot = `SELECT json_build_array((SELECT json_agg(to_jsonb(t) - 'operation_context' - 'settlement_status' - 'settlement_method' - 'cash_collected_at' - 'cash_collected_by' ORDER BY ride_request_id) FROM trips t),(SELECT json_agg(to_jsonb(o) - 'operation_context' - 'expires_at' ORDER BY ride_request_id) FROM ride_offers o))::text`
