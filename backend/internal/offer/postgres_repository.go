...
func (r PostgresRepository) Market(ctx context.Context, rideRequestID uuid.UUID) (Market, error) {
	...
	err := r.db.QueryRowContext(
		ctx,
		`SELECT id, proposed_fare_minor, currency, booking_mode, status FROM ride_requests WHERE id = $1`,
		rideRequestID,
	).Scan(
...
func (r PostgresRepository) Upsert(ctx context.Context, rideRequestID, driverUserID uuid.UUID, amountMinor, minimumMinor, maximumMinor int64, currency string) (Offer, error) {
	...
	if err := tx.QueryRowContext(
		ctx,
		`SELECT currency, rider_user_id, booking_mode, status FROM ride_requests WHERE id = $1 FOR SHARE`,
		rideRequestID,
	).Scan(&actualCurrency, &riderUserID, &bookingMode, &status); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	if err := tx.QueryRowContext(ctx, `
		...
		RETURNING ride_request_id, driver_user_id, amount_minor, currency, created_at, updated_at
...
func (r PostgresRepository) ListForRider(ctx context.Context, rideRequestID, riderUserID uuid.UUID) ([]Offer, error) {
	...
	if err := r.db.QueryRowContext(
		ctx,
		`SELECT id, booking_mode, status FROM ride_requests WHERE id = $1 AND rider_user_id = $2`,
		rideRequestID,
		riderUserID,
	).Scan(&ownedID, &mode, &status); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	rows, err := r.db.QueryContext(ctx, `
		SELECT ride_request_id, driver_user_id, amount_minor, currency, created_at, updated_at
		FROM ride_offers
