...
func (r PostgresRepository) Create(ctx context.Context,riderUserID uuid.UUID,input CreateInput)(Request,error) {
	var proposedAmount any; var proposedCurrency any
	if input.ProposedFare != nil { proposedAmount = input.ProposedFare.AmountMinor; proposedCurrency = input.ProposedFare.Currency }
	var request Request; var amount sql.NullInt64; var currency sql.NullString
	...
	`,uuid.New(),riderUserID,input.Pickup.Latitude,input.Pickup.Longitude,input.Destination.Latitude,input.Destination.Longitude,input.BookingMode,proposedAmount,proposedCurrency,StatusRequested).Scan(
	...
	if amount.Valid && currency.Valid { request.ProposedFare = &Money{AmountMinor:amount.Int64,Currency:currency.String} }
	return request,nil
}
