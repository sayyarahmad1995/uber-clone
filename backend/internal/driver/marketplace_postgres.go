...
func LockMarketplaceEligible(ctx context.Context, tx *sql.Tx, driverID uuid.UUID, serviceCode string) (bool, error) {
	...
	err := tx.QueryRowContext(ctx, `
        ...
        JOIN driver_vehicle_service_enrollments e ON e.vehicle_id = v.id AND e.service_code = $2
	...
	`, driverID, serviceCode).Scan(&locked)
	...
	`, driverID, serviceCode, MarketplaceLocationMaxAge.Seconds()).Scan(&eligible)
	return eligible, err
}
