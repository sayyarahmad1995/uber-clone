...
func TestOperatingSelectionAndAvailability(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO driver_vehicles(id,driver_user_id,make,model,color,license_plate) VALUES ($1,$2,'Toyota','Corolla','White','READY')`, vehicle, id); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || restored.Selection == nil || restored.Selection.VehicleID != vehicle {
		t.Fatalf("selection not persisted: %#v %v", restored, err)
	}
	if _, err := repo.SetOnline(context.Background(), id, true); !errors.Is(err, ErrLocationRequired) {
		t.Fatalf("missing location accepted: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO driver_locations(driver_user_id,latitude,longitude,updated_at) VALUES ($1,24,67,NOW()-INTERVAL '5 minutes')`, id); err != nil {
		t.Fatal(err)
	}
	if _, err := repo.SetOnline(context.Background(), id, true); !errors.Is(err, ErrLocationRequired) {
		t.Fatalf("stale location accepted: %v", err)
	}
	if _, err := db.Exec(`UPDATE driver_locations SET updated_at=NOW() WHERE driver_user_id=$1`, id); err != nil {
		t.Fatal(err)
	}
	...
	t.Cleanup(func() {
		db.Exec(`DELETE FROM trips WHERE ride_request_id=$1`, rideID)
		db.Exec(`DELETE FROM ride_requests WHERE id=$1`, rideID)
	})
	...
	if _, err := repo.SetOnline(context.Background(), id, true); !errors.Is(err, ErrTripActive) {
		t.Fatalf("trip online allowed: %v", err)
	}
	if _, err := repo.SetOnline(context.Background(), id, false); err != nil {
		t.Fatalf("offline blocked by trip: %v", err)
	}
	if _, err := db.Exec(`DELETE FROM trips WHERE ride_request_id=$1`, rideID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`DELETE FROM driver_vehicle_service_enrollments WHERE vehicle_id=$1`, vehicle); err != nil {
		t.Fatal(err)
	}
	if _, err := repo.SetOnline(context.Background(), id, true); !errors.Is(err, ErrSelectionInvalid) {
