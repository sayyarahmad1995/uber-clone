...
func TestRiderReadsUnassignedRequestWithAndWithoutOffer(t *testing.T) {
 for _, withOffer := range []bool{false, true} {
  name := "before_offer"
  if withOffer { name = "after_driver_offer" }
  t.Run(name, func(t *testing.T) {
   ...
   if err != nil { t.Fatalf("get unassigned request: %v", err) }
   if view.Trip != nil { t.Fatal("an offer must not assign a trip") }
   views, err := statuses.ListOwned(ctx, rider, 50)
   if err != nil || len(views) != 1 { t.Fatalf("list unassigned requests: %v, count=%d", err, len(views)) }
   comparison, err := offers.ListForRider(ctx, rideID, rider)
   if err != nil { t.Fatal(err) }
...
func TestLegacyTripWithNullContextRemainsReadableAndExecutable(t *testing.T) {
 ...
 offers := geoOffers(db)
 if _, err := offers.AcceptProposed(ctx, rideID, driverID); err != nil { t.Fatal(err) }
 if _, err := offers.Accept(ctx, rideID, rider, driverID); err != nil { t.Fatal(err) }
 // Pre-migration trips have no captured context. Never fabricate one on read.
 if _, err := db.Exec(`UPDATE trips SET operation_context=NULL WHERE ride_request_id=$1`, rideID); err != nil { t.Fatal(err) }
 t.Run("rider_read",func(t *testing.T) {
  if _, err := ridestatus.NewPostgresRepository(db).GetOwned(ctx, rideID, rider); err != nil { t.Fatal(err) }
 })
 t.Run("driver_read",func(t *testing.T) {
  if _, err := drivertrip.NewPostgresRepository(db).GetCurrent(ctx, driverID); err != nil { t.Fatal(err) }
 })
 t.Run("execution_and_history",func(t *testing.T) {
  ...
  if _, err := trips.Complete(ctx, rideID, driverID); err != nil { t.Fatal(err) }
  history, err := drivertrip.NewPostgresRepository(db).ListHistory(ctx, driverID, 50)
  if err != nil || len(history) != 1 { t.Fatalf("legacy history: %v count=%d", err, len(history)) }
 })
}
